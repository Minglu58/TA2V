...
from tav import Net2NetTransformer, VideoData, AudioTransformer
...
def main():
    ...
    parser = AudioTransformer.add_model_specific_args(parser)
    ...
    model = AudioTransformer(args, first_stage_key=args.first_stage_key, cond_stage_key=args.cond_stage_key)

    callbacks = []
    callbacks.append(ModelCheckpoint(every_n_train_steps=3000, save_top_k=-1, filename='{epoch}-{step}-{train_audio_loss:.2f}-{val_audio_loss:.2f}'))
    #callbacks.append(ModelCheckpoint(monitor='val_loss', mode='min', save_top_k=2, filename='best_checkpoint-{val/loss:.2f}'))
    callbacks.append(ModelCheckpoint(monitor='val_audio_loss', mode='min', save_top_k=2, filename='best_checkpoint-{val_audio_loss:.2f}'))
    callbacks.append(EarlyStopping(monitor = 'val_audio_acc1', patience=30, mode = 'max'))
